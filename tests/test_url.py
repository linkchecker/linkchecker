...
class TestUrl (unittest.TestCase):
    ...
    @need_python2
    def test_norm_quote_latin (self):
        """
        Check quoting URL with Latin 1 characters
        This does not work on Python 3,
        because urllib.parse.quote works only with UTF-8
        """
    ...
    def test_norm_quote_utf (self):
        """ Check quoting the same URL with UTF-8 characters """
        url = u"http://localhost:8001/?quoted=ü"
        nurl = "http://localhost:8001/?quoted=%C3%BC"
        self.urlnormtest(url, nurl, encoding="utf-8")
